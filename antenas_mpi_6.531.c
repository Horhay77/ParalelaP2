...
/**
 * Actualizar el mapa con la nueva antena. En esta versión, si la antena a colocar
 * no pertenece a nuestra sección del mapa pararemos de actualizar en cuanto una fila
 * no haya sido modificada.
 */
void actualizar(int * mapa, int offset, int tam, int cols, Antena antena){
	if(antena.fila >= offset && antena.fila < offset+tam){	
		for(int i=0; i<tam; i++){
			for(int j=0; j<cols; j++){
	...
	else if(antena.fila < offset){
		// Hacemos el recorrido desde la fila superior, ya que estamos
		// por debajo de la nueva antena
		int updated_row;
		for(int i=0; i<tam; i++){
			updated_row=0;
			for(int j=0; j<cols; j++){
		...
		// Hacemos el recorrido desde la fila inferior, ya que estamos
		// por encima de la nueva antena
		int updated_row;
...
int main(int nargs, char ** vargs){
	...
	*/
	...
	// Reparto de las filas en función del número del proceso que seas:
	int offset;
	int lastrow;
	int tam;
	printf("SIZE:%d\n",size);
	if(size != 20){
		...
		// Otorgar a 12 procesos más carga de forma aleatoria a ver si hay suerte y cae
		// el trabajo en la máquina de hydra. (HAIL HYDRA)
		// Idea: Pensar que hay 32 procesos -> los 12 primeros toman 
		// la carga de 24 procesos y los 8 restantes lo que queda.
		int carga = 32;
		if(rows >= carga){
			if(rank < 12){
				offset = 2* rank * rows / carga;
				lastrow = (2 * (rank + 1) * rows / carga)-1;
				tam = lastrow - offset + 1;
			}
			else{
				offset = (rank + 12) * rows / carga;
				lastrow = ((rank+13)*rows /carga)-1;
				tam = lastrow - offset +1;
			}
		}
		else{
